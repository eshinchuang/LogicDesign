To extract the tar-gzipped file, use the following Linux/UNIX command:
  ...
  $ tar zxvf da3_template.tgz

File list:
----------
00_README.txt       : This README file.
ant.sh              : Shell script to simulate the maze example.
ant_fsdb.sh         : Shell script to simulate the maze example with fsdb output.
...
maze_universe.v     : Maze Universe that reacts to your Ant-Person suit.
ant_suit.v          : Ant-Person suit that you are going to design and
                    : replace with.
...
There is a maze example for challange mode:
...
c. For the narrow corridor challenge:
   ...
   加上 +define+NOCHECK 把迷宮檢查關掉。或是在模擬時包含 nocheck.v

d. For the wall island challenge:

   加上 +define+CHALLENGE 引入修改後的 IO. 或是在模擬時包含 challenge.v
